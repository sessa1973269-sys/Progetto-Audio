...
if os.path.exists(target_file) and os.path.exists(interf_file):
    print("\n✓ File audio trovati! Carico segnali reali...")
    ...
    # Controllo compatibilità frequenze di campionamento
    if fs_t != fs or fs_i != fs:
        print("\n⚠️ Frequenze di campionamento non compatibili!")
        print(f"   Target: {fs_t} Hz, Interferente: {fs_i} Hz, Atteso: {fs} Hz")
        print("   → Converto automaticamente in 16 kHz? No (non implementato).")
        print("   → Uso segnali sintetici di test.")
        use_generated = True

else:
    print("\n⚠️ File audio NON trovati nella cartella del progetto.")
    print("   → Uso segnali sintetici di test.")
    use_generated = True
...
if use_generated:
    ...
    print("\n✓ Generati segnali sintetici di test.")
